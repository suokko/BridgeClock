...
function getSlices(numberOfSlices,R){
    ...
    for (var i=0;i<numberOfSlices;i++){
        slices.push(
                    {
                        x:R-Math.sin(2*i*Math.PI/numberOfSlices)*R,
                        y:R-Math.cos(2*i*Math.PI/numberOfSlices)*R
                    })
    }
    return slices;
}
function findNearestSlice(x,y, numberOfSlices, R){
    var slices = getSlices(numberOfSlices, R)
    var minDist=1000000;
    var nearest=-1;
